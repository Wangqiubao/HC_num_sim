%%%Figure 9. Probability density diagram with noise intensity K as bifurcation parameter when tau = 12.562. (a)K = 1, (b)K = 200%%%
clear 
clc
...
for i = m: NumberOfSubinterval
 ...
 Rsolution(:,i+1)=Rsolution(:,i)+Stepsize.*(rho.*Asolution(:,i)+gamma2.*Qsolution(:,i)-mu.*Rsolution(:,i));
   ...
   figure(7)  
   for i=1:200001 
       x(:,i)=Solution(:,300000+i-1);
   end 
    for i=1:200001 
            y(:,i)=Qsolution(:,300000+i-1);
    end 

 xxx=[mean(x)/N;mean(y)/N];
     subplot(2,2,1)
       ksdensity(mean(x)/N);
     h1=xlabel('$S/N$','FontSize',15);
  h2=ylabel('$Density$','FontSize',15);
